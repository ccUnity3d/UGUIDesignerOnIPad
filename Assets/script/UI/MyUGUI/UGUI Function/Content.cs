...
public class Content : MonoBehaviour{
	...
	private List<ContentItem> listItem;
	private Queue<ContentItem> useItem;
	...
	public void Awake()
	{
		listItem = new List<ContentItem> ();
		useItem = new Queue<ContentItem> ();
        contentTrans = transform.GetComponent<RectTransform>();
        itemPrefab = transform.parent.parent.parent.FindChild("itemSkin").gameObject;
    }
	...
	void CreateItem (int dataIndex)
	{
		...
		if (useItem.Count >0) {
            ...
            item = AddChild(itemPrefab, contentTrans).GetComponent<ContentItem>(); 
		}
        item.MyContent = this;
