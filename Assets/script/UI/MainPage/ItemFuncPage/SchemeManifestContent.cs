...
public class SchemeManifestContent : MonoBehaviour{
	...
	private List<SchemeManifestContentItem> listItem;
	private Queue<SchemeManifestContentItem> useItem;
	...
	public void Awake()
	{
		listItem = new List<SchemeManifestContentItem> ();
		useItem = new Queue<SchemeManifestContentItem> ();
        contentTrans = transform.GetComponent<RectTransform>();
        itemPrefab = transform.parent.parent.FindChild("OfferItemSkin").gameObject;
    }
    ...
    void SetUpdataRectItem(int scrollPerLineIndex)
    {
        ...
        {
            SchemeManifestContentItem item = listItem[i];
	...
	{
		SchemeManifestContentItem item;
		if (useItem.Count >0) {
			item = useItem.Dequeue ();
		} else {
            item = AddChild(itemPrefab, contentTrans).GetComponent<SchemeManifestContentItem>(); 
		}
        item.MyContent = this;
