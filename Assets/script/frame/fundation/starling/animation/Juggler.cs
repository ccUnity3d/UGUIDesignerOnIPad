...
namespace starling
{
    ...
    public class Juggler : ITickable
    {
		
		List<ITickable> mObjects;
		float mElapsedTime;
		...
		public void add(ITickable obj)
		{
		    if (obj != null && mObjects.IndexOf(obj) == -1) 
		    {
		        ...
		        if (dispatcher != null) dispatcher.addEventListener(EventX.REMOVE_FROM_JUGGLER, onRemove);
		    }
		}
		
		/** Determines if an object has been added to the juggler. */
		public bool contains(ITickable obj)
		{
		...
		public void remove(ITickable obj)
		{
		    ...
		    var dispatcher = obj as EventDispatcher;
		    if (dispatcher != null) dispatcher.removeEventListener(EventX.REMOVE_FROM_JUGGLER, onRemove);
		...
		public void removeTweens(object target)
		{
		    ...
		    for (var i=mObjects.Count-1; i>=0; --i)
		    {
		        ...
		        {
		            tween.removeEventListener(EventX.REMOVE_FROM_JUGGLER, onRemove);
		            mObjects.RemoveAt(i);
		        }
		    }
		}
		...
		public void purge()
		{
		    ...
		    for (var i=mObjects.Count-1; i>=0; --i)
		    {
		        ...
		        if (dispatcher != null)
		        {
		            dispatcher.removeEventListener(EventX.REMOVE_FROM_JUGGLER, onRemove);
		        }
		        mObjects.RemoveAt(i);
		    }
		}
		...
		public void tween(object target, float time, TweenParams properties)
		{
			...
			{		
		        if ( hasProperty(tween, key) )
					SetValue(tween, key, properties[key]);
				else if ( hasProperty(target, key) )
		           	tween.animate(key, (float)properties[key]);
				else
		            throw new ArgumentException("Invalid property: " + key);
			}
		    
		    tween.addEventListener(EventX.REMOVE_FROM_JUGGLER, onPooledTweenComplete);
		    add(tween);
		}
		...
		private void onPooledTweenComplete(EventX e)
		{
		...
		private void onRemove(EventX e)
		{
		    remove(e.target as ITickable);
