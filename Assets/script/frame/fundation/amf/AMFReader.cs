    ...
    public class AMFReader : BinaryReader
    {
            ...
            if (inline)
            {
                ...
                if (@fixed == 1)
                {
                    ...
                    for (int i = 0; i < handle; i++)
                    {
                        ...
                        list[i]= TypeHelper.ChangeType(obj, type); ;
                    }
                    return list;
                }
                else
                {
                    ...
                    for (int i = 0; i < handle; i++)
                    {
                        byte typeCode = this.ReadByte();
                        object obj = ReadAMF3Data(typeCode);
                        obj=TypeHelper.ChangeType(obj, type);
                        list.Add(obj);
                    }
                    return list;
                }
        ...
        internal void SetMember(object instance, string memberName, object value)
        {
            ...
            if (propertyInfo != null)
            {
                ...
                {
                    throw new Exception("amf class:" + instance.GetType().Name + " property:" + propertyInfo.Name + " newValue typeof：" + value.GetType() + " msg:" + ex.Message);
                }
            }
            else
            {
                ...
                try
                {
                    ...
                    {
                        notFoundMessage.AppendLine("amf class:" + instance.GetType().Name + " field:" + memberName + " not found!");
                    }
                }
                catch (Exception ex)
                {
                    throw new Exception("amf class:" + instance.GetType().Name + " field:" + fi.Name + " newValue typeof：" + value.GetType()+" msg:"+ ex.Message);
                }
            }
        }
